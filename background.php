...
function source_map($options)
{
	global $User_Vars;
	$folder = key_or_default($options, "folder", null);
	$image = key_or_default($options, "default", "");
	$extension = key_or_default($options, "extension", "jpg");

	if($folder != null && $User_Vars["mapname"] != "nomapname")
	{
		$tempImage = $folder . "/" . $User_Vars["mapname"] . "." . $extension;
		if(substr($folder, 0, 7) == "http://")
		{
			$image = $tempImage;
		}
		else if(file_exists($tempImage))
		{
			$image = $tempImage;
		}
	}

	echo "background: url(" . $image . ");background-position:center;background-size:cover;background-attachment:fixed;";
}
$sources["map"] = "source_map";
